...
class SigningBeneficiariesModal extends Component
{
            ...
            try {
                foreach ($this->selectedRows as $key => $row) {
                    if (isset($this->beneficiaries[$key])) {
                        ...
                        $this->authorize('check-coordinator', $beneficiary->batch);
                ...
                if ($marked_count > 0 && $saved) {
                    ...
                    $this->dispatch('alertNotification', type: null, message: 'Successfully marked beneficiaries for COS', color: 'blue');
                    ...
                    $this->dispatch('alertNotification', type: null, message: 'Successfully unchecked all beneficiaries from COS', color: 'blue');
                }
        ...
        DB::transaction(function () {
            ...
            try {
                ...
                if ($marked_count > 0 && $saved) {
                    ...
                    $this->dispatch('alertNotification', type: null, message: 'Successfully marked beneficiaries for payroll', color: 'blue');
                } elseif ($marked_count === 0 && $saved) {
                    LogIt::set_uncheck_beneficiaries_for_payroll($beneficiary->batch->implementation, $beneficiary->batch, auth()->user());
                    $this->dispatch('alertNotification', type: null, message: 'Successfully unchecked all beneficiaries from payroll', color: 'blue');
                }
            } catch (AuthorizationException $e) {
                DB::rollBack();
                LogIt::set_log_exception('An unauthorized action has been made while adding a beneficiary. Error ' . $e->getCode(), auth()->user(), $e->getTrace());
    ...
    {
        return view('livewire.coordinator.submissions.signing-beneficiaries-modal');
    }
}
