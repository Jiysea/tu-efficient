...
<div x-data="{
    ...
    archiveBeneficiaryModal: $wire.entangle('archiveBeneficiaryModal'),
    promptMultiArchiveModal: $wire.entangle('promptMultiArchiveModal'),
    viewCredentialsModal: $wire.entangle('viewCredentialsModal'),
    submitBatchModal: $wire.entangle('submitBatchModal'),
    exitBatchModal: $wire.entangle('exitBatchModal')
}" class="p-2 min-h-screen select-none text-green-1100">

    {{-- App Name | Submission Type | Submit Button | Exit Button --}}
    <div class="relative flex flex-col sm:flex-row items-center justify-between mb-2">

        {{-- App Name && Submission Type --}}
        <div class="flex items-center justify-center w-full sm:w-auto">
            ...
            <h1 class="text-3xl sm:text-xl md:text-3xl font-bold ms-3 text-green-900 drop-shadow">TU-EFFICIENT</h1>
        ...
        <div class="relative w-full sm:w-auto flex items-center justify-end gap-3">
            ...
            {{-- Loading State --}}
            <svg class="absolute right-full me-3 size-6 z-50 text-green-900 animate-spin" wire:loading
                wire:target="selectBeneficiaryRow, selectShiftBeneficiary, openEdit, loadMoreBeneficiaries, deleteBeneficiary, removeBeneficiaries"
            ...
            <span class="relative" x-data="{ pop: false }">
                <button type="button" id="submitBatchButton" @mouseleave="pop = false;" @mouseenter="pop = true;"
                    @if ($this->beneficiaryCount === $this->batch?->slots_allocated) @click="submitBatchModal = true;"
                    @else disabled @endif
                    class="flex items-center text-xs sm:text-sm disabled:bg-gray-300 disabled:text-gray-500 bg-green-700 hover:bg-green-800 active:bg-green-900 text-green-50 rounded-md px-4 py-2 font-bold">
                    SUBMIT
                ...
                <div x-cloak x-show="pop" x-transition.opacity
                    class="absolute z-50 top-full mt-2 right-0 rounded p-2 shadow text-xs md:text-sm font-normal whitespace-nowrap border bg-zinc-900 border-zinc-300 text-green-50">
                    @if ($this->beneficiaryCount === $this->batch?->slots_allocated)
                        Submit this batch
                    @else
                        You cannot <span class="text-green-700">submit</span> this batch until<br>
                        the amount of beneficiaries is full.
                    @endif
                </div>
            </span>
            ...
            {{-- Exit Button --}}
            <span class="relative" x-data="{ pop: false }">
                <button type="button" id="exitBatchModal" @mouseleave="pop = false;" @mouseenter="pop = true;"
                    @click="exitBatchModal = true;"
                    class="flex items-center text-xs sm:text-sm bg-red-700 hover:bg-red-800 active:bg-red-900 text-red-50 rounded-md p-2 font-bold">
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" class="size-5" xmlns:xlink="http://www.w3.org/1999/xlink"
                        ...
                        <g>
                            <path
                                d="M131.641 5.216 C 65.755 7.899,33.637 23.506,14.181 62.293 C -0.043 90.650,-4.849 199.264,5.125 266.935 C 10.309 302.103,31.150 338.805,55.305 355.300 C 89.749 378.823,143.518 397.935,165.816 394.582 C 183.497 391.922,201.161 373.298,205.211 353.044 C 205.793 350.135,206.731 349.671,214.453 348.467 C 254.481 342.228,284.737 310.107,291.382 266.797 C 297.000 230.183,294.544 220.312,279.816 220.313 C 268.882 220.313,265.411 224.964,263.678 241.938 C 262.041 257.978,260.356 269.355,258.884 274.312 C 252.827 294.710,232.358 314.014,212.962 317.621 L 209.908 318.189 210.409 309.681 C 214.184 245.533,211.321 171.755,203.924 142.578 C 196.685 114.023,177.579 83.114,159.020 69.934 C 146.378 60.957,125.439 49.778,108.659 43.048 C 97.967 38.759,97.942 38.950,109.375 37.522 C 147.112 32.809,205.750 36.738,223.811 45.190 C 248.253 56.628,259.339 75.178,262.867 110.547 C 265.197 133.901,268.168 138.987,279.517 139.045 C 290.305 139.101,295.024 132.743,294.279 119.152 C 291.541 69.147,277.717 41.561,245.647 22.104 C 225.248 9.728,181.175 3.199,131.641 5.216 M330.078 111.287 C 329.648 111.449,328.337 111.794,327.163 112.052 C 319.511 113.738,314.643 125.452,318.436 133.047 C 319.123 134.422,326.063 141.963,333.858 149.805 L 348.031 164.063 297.882 164.063 C 264.663 164.063,246.394 164.350,243.766 164.913 C 231.198 167.609,227.697 184.074,238.017 191.947 L 240.893 194.141 294.674 194.531 L 348.454 194.922 333.913 209.375 C 318.553 224.643,317.188 226.550,317.188 232.737 C 317.188 243.813,327.468 250.767,338.035 246.839 C 340.525 245.913,397.151 189.816,398.879 186.562 C 400.403 183.694,400.321 175.557,398.736 172.422 C 397.050 169.088,344.222 116.019,340.234 113.653 C 337.293 111.909,331.811 110.631,330.078 111.287 "
    ...
    {{-- Content --}}
    <div class="relative flex flex-col lg:grid lg:grid-cols-8 gap-4">
            ...
            <div class="flex flex-col gap-2 mb-2">
                ...
                <div class="flex flex-1 flex-col justify-center">
                    ...
                    <span class="flex items-center justify-start gap-2 font-bold text-lg my-2 ms-2 text-green-900">
                        ...
                        Beneficiaries
                        <span class="relative" x-data="{ pop: false }">
                            <span class="rounded text-xs px-2 py-1 font-medium" @mouseleave="pop = false;"
                                @mouseenter="pop = true;"
                                :class="{
                            ...
                            <div x-cloak x-show="pop" x-transition.opacity
                                class="absolute z-50 bottom-full mb-2 -left-20 rounded p-2 shadow text-xs md:text-sm font-normal whitespace-nowrap border bg-zinc-900 border-zinc-300 text-green-50">
                                This is the <span class="text-green-700">current amount of
                                    beneficiaries</span>
                                inserted <br>
                                and the <span class="text-green-700">total allocated slots</span> for
                                this batch.
                            </div>
                        </span>
                    </span>
                ...
                <div class="flex items-center justify-end mx-2 gap-2">
                    ...
                    <span class="relative" x-data="{ pop: false }">
                        <button id="addModalButton" @mouseleave="pop = false;" @mouseenter="pop = true;"
                            @if ($this->beneficiaryCount === $this->batch?->slots_allocated) disabled
                                    @else
                                    @click="addBeneficiariesModal = !addBeneficiariesModal;" @endif
                            class="flex items-center disabled:bg-gray-300 disabled:text-gray-500 bg-green-700 hover:bg-green-800 active:bg-green-900 text-green-50 rounded-md px-4 py-1 text-sm font-bold focus:ring-green-500 focus:border-green-500 focus:outline-green-500 duration-200 ease-in-out">
                            ADD
                        ...
                        <div x-cloak x-show="pop" x-transition.opacity
                            class="absolute z-50 bottom-full mb-2 right-0 rounded p-2 shadow text-xs md:text-sm font-normal whitespace-nowrap border bg-zinc-900 border-zinc-300 text-green-50">
                            @if ($this->beneficiaryCount === $this->batch?->slots_allocated)
                                You cannot add more beneficiaries<br>
                                when the <span class="text-green-700">slots is full</span>.
                            @else
                                Add beneficiaries
                            @endif
                        </div>
                    ...
                    <table class="relative w-full text-sm text-left text-green-1100">
                        ...
                        <tbody x-data="{ count: 0 }" class="text-xs">
                            @foreach ($this->beneficiaries as $key => $beneficiary)
                                <tr wire:key="beneficiary-{{ $key }}" @click="count++"
                                    ...
                                    :class="{
                                        'bg-gray-100 text-green-1000 hover:bg-gray-50': {{ json_encode($beneficiary->beneficiary_type === 'underemployed' && in_array($key, $selectedBeneficiaryRow)) }},
                                    ...
                                    <td class="absolute h-full w-1 left-0 z-50"
                                        :class="{
                                            'bg-green-700': {{ json_encode($beneficiary->beneficiary_type === 'underemployed' && in_array($key, $selectedBeneficiaryRow)) }},
                                    ...
                                    @if (count($selectedBeneficiaryRow) > 0)
                                        <th scope="row" class="px-2 text-center">
                                            <label tabindex="0" @click.stop
                                                class="relative flex flex-1 items-center justify-center gap-1 rounded p-1 outline-none border-2 cursor-pointer"
                                                :class="{
                                                    'bg-green-800 border-green-800 text-green-50': {{ json_encode($beneficiary->beneficiary_type === 'underemployed' && in_array($key, $selectedBeneficiaryRow)) }},
                                                ...
                                                <input id="check-beneficiary-{{ $key }}" type="checkbox"
                                                    tabindex="-1" value={{ $key }}
        ...
        <div class="relative flex flex-col col-span-4 size-full">
            <div class="flex flex-col size-full rounded bg-white shadow text-xs select-text">
                ...
                @if (count($selectedBeneficiaryRow) === 1)
                    {{-- Whole Thing --}}
                    <div class="grid grid-cols-11 gap-2 p-4 select-none">

                        {{-- Left Side --}}
                        <div
                            class="hidden sm:flex flex-col items-center col-span-full sm:col-span-3 order-2 sm:order-none text-green-1100 gap-2">
                            ...
                            <div class="flex flex-col items-center text-green-1100">
                                ...
                                <div
                                    ...
                                    @if ($this->identity)
                                        <button tabindex="-1"
                                            ...
                                            <svg class="absolute flex items-center justify-center m-0 size-6 text-green-900 animate-spin z-10"
                                                wire:loading wire:target="viewCredential('identity')"
                                ...
                                {{-- Type of ID --}}
                                <p
                                    class="whitespace-nowrap overflow-x-auto scrollbar-none selection:bg-green-700 selection:text-green-50 font-semibold select-text text-center">
                                    {{ $this->getIdType }}
                                </p>

                                {{-- ID Number --}}
                                <p
                                    class="whitespace-nowrap overflow-x-auto scrollbar-none selection:bg-green-700 selection:text-green-50 text-center select-text">
                            ...
                            <div class="flex flex-col w-full text-green-1100 gap-1">
                                ...
                                <div class="flex flex-1 flex-col px-2 py-1 gap-2">
                                    ...
                                    <div class="flex flex-1 flex-col justify-center">
                                        <p
                                            class="select-all selection:bg-green-700 selection:text-green-50 font-medium capitalize">
                                            province </p>
                                        <span
                                            class="whitespace-nowrap overflow-x-auto scrollbar-none selection:bg-green-700 selection:text-green-50 bg-green-50 text-green-1000 rounded px-2 py-0.5 select-text">
                        ...
                        {{-- Identity Information --}}
                        <div
                            class="sm:hidden flex flex-col items-center col-span-full sm:col-span-3 order-1 sm:order-none text-green-1100">
                        ...
                        {{-- Address Information --}}
                        <div
                            class="sm:hidden flex flex-col w-full col-span-full sm:col-span-3 sm:col-start-1 order-3 text-green-1100 gap-1">
                        ...
                        {{-- Spouse Information --}}
                        <div
                            class="sm:hidden flex flex-col w-full col-span-full sm:col-span-3 order-4 sm:order-none text-green-1100 gap-1">
                        ...
                        {{-- Right Side --}}
                        <div class="flex flex-col col-span-full sm:col-span-8 order-2 text-green-1100 gap-1">
                            ...
                            <div class="flex flex-1 flex-col px-2 py-1 gap-2">
                                ...
                                <div class="flex items-center whitespace-nowrap justify-between gap-2">
                                    ...
                                    <div class="flex flex-1 flex-col justify-center">
                                        <p
                                            ...
                                            sex </p>
                                        <span
                                            class="whitespace-nowrap overflow-x-auto scrollbar-none selection:bg-green-700 selection:text-green-50 bg-green-50 text-green-1000 rounded px-2 py-0.5 capitalize select-text">
                                ...
                                <div class="flex items-center whitespace-nowrap justify-between gap-2">
                                    ...
                                    <div class="flex flex-1 flex-col justify-center">
                                        <p
                                            class="select-all selection:bg-green-700 selection:text-green-50 font-medium">
                                        ...
                                        @if ($this->beneficiary->beneficiary_type === 'special case')
                                            <button type="button" @click="$wire.viewCredential('special');"
                                                class="relative flex items-center justify-between whitespace-nowrap overflow-x-auto scrollbar-none selection:bg-green-700 selection:text-green-50 rounded capitalize px-2 py-0.5 outline-none bg-red-100 active:text-red-200 text-red-950 hover:text-red-700 duration-200 ease-in-out">
                                        ...
                                        @else
                                            <span
                                                class="whitespace-nowrap overflow-x-auto scrollbar-none selection:bg-green-700 selection:text-green-50 rounded px-2 py-0.5 bg-green-50 text-green-1000 capitalize select-text">
                                                {{ $this->beneficiary->beneficiary_type }}
                                            </span>
                                        @endif
                            ...
                            <div class="hidden sm:flex flex-1 px-2 py-1 gap-2">
                                <div class="relative max-[430px]:flex-col flex flex-1 items-center justify-end gap-2">
                                    ...
                                    {{-- Trash/Delete Button --}}
                                    <button
                                        @if ($this->batch->approval_status !== 'approved') @click="archiveBeneficiaryModal = !archiveBeneficiaryModal;"
                                        @else
                                        disabled @endif
                ...
                @elseif(count($selectedBeneficiaryRow) > 1)
                    <div class="rounded relative bg-white p-4 size-full flex items-center justify-center">
                        <div
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" class="size-20 mb-4 text-green-900 opacity-65"
                                xmlns:xlink="http://www.w3.org/1999/xlink" width="400" height="400"
                                viewBox="0, 0, 400,400">
                                <g>
                                    <path
                            ...
                            <span class="flex items-center justify-center gap-2 mt-2">
                                <button type="button" @click="promptMultiArchiveModal = true;"
    ...
    {{-- Prompt Multi Archive Modal --}}
    <livewire:barangay.listing-page.prompt-multi-archive-modal :$beneficiaryIds />

    {{-- Archive Beneficiary Modal --}}
    <div x-cloak class="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm z-50" x-show="archiveBeneficiaryModal"
        @keydown.window.escape="archiveBeneficiaryModal = false">

        <!-- Modal -->
        <div x-show="archiveBeneficiaryModal" x-trap.noautofocus.noscroll="archiveBeneficiaryModal"
            ...
            <div class="relative">
                <div class="relative bg-white rounded-md shadow overflow-hidden">
                    ...
                    <div class="flex items-center justify-between py-2 px-4 rounded-t-md">
                        ...
                        {{-- Close Button --}}
                        <button type="button" @click="archiveBeneficiaryModal = false;"
                    ...
                    <div class="grid w-full place-items-center py-5 px-16 text-green-1100 text-xs">
                        ...
                        <div class="flex items-center justify-center w-full gap-2">
                            <button type="button"
                                ...
                                @click="archiveBeneficiaryModal = false;">
                                CANCEL
                            </button>
                            <button type="button"
                                ...
                                @click="$wire.deleteBeneficiary(); archiveBeneficiaryModal = false;">
                                CONFIRM
                            </button>
                        </div>
                    </div>
    ...
    {{-- Submit Batch Modal --}}
    <div x-cloak @keydown.window.escape="submitBatchModal = false">
        <!-- Modal Backdrop -->
                ...
                <div class="relative bg-white rounded-md shadow">
                    ...
                    <div class="relative flex items-center justify-between py-2 px-4 rounded-t-md">
                        ...
                        <div class="flex items-center justify-center">
                            ...
                            <div class="z-50 text-green-900" wire:loading wire:target="confirm_submit, submitBatch">
                            ...
                            <button type="button" @click="$wire.resetConfirm(); submitBatchModal = false;"
                                class="outline-none text-green-400 hover:bg-green-200 hover:text-green-900 focus:bg-green-200 focus:text-green-900 rounded size-8 ms-auto inline-flex justify-center items-center duration-300 ease-in-out">
                                <svg class="size-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                    ...
                    {{-- Modal body --}}
                    <form wire:submit.prevent="submitBatch"
                        ...
                        <p class="mb-4 text-xs font-normal text-gray-500">You won't be able to access this until
                        ...
                        <div class="relative flex items-center justify-center w-full gap-2">
                            <div class="relative">
                                <input type="text" id="confirm_submit" wire:model.blur="confirm_submit"
                                    ...
                                    class="flex flex-1 {{ $errors->has('confirm_submit') ? 'caret-red-900 border-red-500 focus:border-red-500 bg-red-100 text-red-700 placeholder-red-500' : 'caret-green-900 border-green-300 focus:border-green-500 bg-green-50' }} focus:ring-0 rounded outline-none border py-2.5 text-sm select-all selection:bg-green-700 selection:text-green-50 duration-200 ease-in-out"
                            ...
                            <button type="submit" wire:loading.attr="disabled" wire:target="submitBatch"
                                class="outline-none flex items-center justify-center disabled:bg-green-300 bg-green-700 hover:bg-green-800 active:bg-green-900 focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-green-500 text-green-50 py-2.5 px-2 rounded text-sm font-bold duration-200 ease-in-out">
                                CONFIRM
                            </button>
                        </div>
                    </form>
    ...
    {{-- Exit Batch Modal --}}
    <div x-cloak @keydown.window.escape="exitBatchModal = false">
        <!-- Modal Backdrop -->
        <div class="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm z-50" x-show="exitBatchModal">
        </div>

        <!-- Modal -->
        <div x-show="exitBatchModal" x-trap.noautofocus.noscroll="exitBatchModal"
            class="fixed inset-0 p-4 flex items-center justify-center overflow-y-auto z-50 select-none h-[calc(100%-1rem)] max-h-full">
            ...
            <div class="relative w-full max-w-xl max-h-full">
                <div class="relative bg-white rounded-md shadow">
                    ...
                    <!-- Modal Header -->
                    <div class="relative flex items-center justify-between py-2 px-4 rounded-t-md">
                        <h1 class="text-sm sm:text-base font-semibold text-green-1100">Exit from this Submission
                        </h1>

                        <div class="flex items-center justify-center">
                            {{-- Loading State for Changes --}}
                            <div class="z-50 text-green-900" wire:loading wire:target="exitBatch">
                                <svg class="size-6 mr-3 -ml-1 animate-spin" xmlns="http://www.w3.org/2000/svg"
                                    fill="none" viewBox="0 0 24 24">
                                    <circle class="opacity-25" cx="12" cy="12" r="10"
                                        stroke="currentColor" stroke-width="4">
                            ...
                            {{-- Close Modal --}}
                            <button type="button" @click="exitBatchModal = false;"
                    ...
                    {{-- Modal body --}}
                    <div class="grid w-full place-items-center py-5 px-3 md:px-16 text-xs">

                        <p class="mb-2 text-sm font-medium text-green-1100">Are you sure you want to exit?
                        </p>
                        <p class="mb-4 text-xs font-normal text-gray-500">You can access this submission again with
                            the same code later.
                        </p>

                        <div class="relative flex items-center justify-center w-full gap-2">
                            <button type="button" wire:loading.attr="disabled" wire:target="exitBatch"
                                class="flex items-center justify-center border border-green-700 hover:border-transparent active:border-transparent hover:bg-green-800 active:bg-green-900 text-green-700 hover:text-green-50 active:text-green-50 py-2.5 px-2 rounded text-sm font-bold duration-200 ease-in-out"
                                @click="exitBatchModal = false;">
                                CANCEL
                            </button>
                            <button type="submit" wire:loading.attr="disabled" wire:target="exitBatch"
                                class="flex items-center justify-center border border-green-700 hover:border-green-800 active:border-green-900 bg-green-700 hover:bg-green-800 active:bg-green-900 text-green-50 py-2.5 px-2 rounded text-sm font-bold duration-200 ease-in-out"
                                wire:click="exitBatch">
                                CONFIRM
                            </button>
                        </div>
                    </div>
        ...
        <template x-for="alert in alerts" :key="alert.id">
            <div x-show="show" x-data="{ show: false }" x-init="$nextTick(() => { show = true });"
                ...
                <button type="button" @click="removeAlert(alert.id)" class="p-1">
                    <svg xmlns="http://www.w3.org/2000/svg" class="size-4" xmlns:xlink="http://www.w3.org/1999/xlink"
                        width="400" height="400" viewBox="0, 0, 400,400">
                        <g>
                            <path
