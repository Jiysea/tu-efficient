...
window.matchMedia('(min-width: 1280px)').addEventListener('change', event => {
        ...
        'xl:ml-64': open === true,
            ...
            <div class="p-2 min-h-screen select-none">
                ...
                <div class="relative flex items-center my-2">
                    ...
                    {{-- Date range picker --}}
                    <div id="assignments-date-range" date-rangepicker datepicker-autohide class="flex items-center gap-2 text-blue-1100">
                    ...
                    {{-- Loading State --}}
                    <div class="absolute items-center justify-end z-50 min-h-full min-w-full text-blue-900" wire:loading.flex>
                ...
                <div class="relative grid grid-cols-1 w-full h-full gap-4 lg:grid-cols-5">
                    ...
                    <div x-data="{ viewBatchModal: $wire.entangle('viewBatchModal') }" class="relative lg:col-span-3 h-full w-full rounded bg-white shadow">
                        ...
                        <div class="relative max-h-12 flex items-center justify-between">
                            ...
                            <div class="inline-flex items-center text-blue-900">
                                ...
                                <span class="font-medium text-xs px-2 py-1 rounded bg-blue-100 text-blue-700">{{ $batchesCount ?? 0 }}</span>
                            </div>
                            ...
                            <div class="mx-2 flex items-center justify-end">
                                <div class="relative me-2">
                                    <div class="absolute inset-y-0 start-0 flex items-center ps-2 pointer-events-none">
                                        <svg class="size-3 text-blue-500" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 20">
                                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m19 19-4-4m0-7A7 7 0 1 1 1 8a7 7 0 0 1 14 0Z" />
                                        </svg>
                                    </div>
                                    <input type="text" id="batch-search" maxlength="100" autocomplete="off" wire:model.live.debounce.350ms="searchBatches" class="duration-200 outline-none ease-in-out ps-7 py-1 text-xs text-blue-1100 placeholder-blue-500 border border-blue-300 rounded w-full bg-blue-50 focus:ring-blue-500 focus:border-blue-500" placeholder="Search for batch numbers">
                                </div>

                                {{-- Filter Button --}}
                                <div x-cloak x-data="{ open: false }" class="relative">
                                    <!-- Button -->
                                    <button x-ref="button" @click="open = !open" :aria-expanded="open" type="button" class="flex items-center bg-blue-700 hover:bg-blue-800 active:bg-blue-900 text-blue-50 hover:text-blue-100 active:text-blue-200 focus:ring-blue-500 focus:border-blue-500 focus:outline-blue-500 rounded px-3 py-1 text-sm font-bold duration-200 ease-in-out">
                                        FILTER
                        ...
                        {{-- Batches Table --}}
                        <div id="batches-table" class="relative min-h-[82.5vh] max-h-[82.5vh] overflow-y-auto overflow-x-auto scrollbar-thin scrollbar-track-white scrollbar-thumb-blue-700">
                            <table class="relative w-full text-sm text-left text-blue-1100 whitespace-nowrap">
                                ...
                                <tbody class="relative text-xs">
                                    ...
                                    <tr x-data="{ row: $wire.entangle('selectedBatchRow') }" wire:key="batch-{{ $key }}" wire:loading.class="pointer-events-none" @click="row == {{ $key }}; $wire.selectBatchRow({{ $key }}, '{{ encrypt($batch->id) }}'); $dispatch('scroll-top-beneficiaries');" class="relative border-b whitespace-nowrap duration-200 ease-in-out cursor-pointer" :class="{
                                        ...
                                        <td class="pr-2 py-2 text-center">
                                            ...
                                            <span class="bg-green-300 text-green-1000 rounded-full text-xs py-1 px-2 uppercase font-semibold">{{ $batch->approval_status }}</span>
                                            @elseif($batch->approval_status === 'pending')
                                            <span class="bg-amber-300 text-amber-900 rounded-full text-xs py-1 px-2 uppercase font-semibold">{{ $batch->approval_status }}</span>
                                            @endif
                                        </td>
                                        <td class="pr-2 py-2 text-center">
                                            @if ($batch->submission_status === 'unopened')
                                            <span class="bg-amber-200 text-amber-900 rounded-full text-xs py-1 px-2 uppercase font-semibold">{{ $batch->submission_status }}</span>
                                            @elseif($batch->submission_status === 'encoding')
                                            <span class="bg-sky-200 text-sky-900 rounded-full text-xs py-1 px-2 uppercase font-semibold">{{ $batch->submission_status }}</span>
                                            @elseif($batch->submission_status === 'submitted')
                                            <span class="bg-green-200 text-green-1000 rounded-full text-xs py-1 px-2 uppercase font-semibold">{{ $batch->submission_status }}</span>
                                            @elseif($batch->submission_status === 'revalidate')
                                            <span class="bg-red-200 text-red-900 rounded-full text-xs py-1 px-2 uppercase font-semibold">{{ $batch->submission_status }}</span>
                                            @endif
                                        </td>

                                        {{-- Batch View --}}
                                        <td class="py-1">
                                            <button type="button" wire:loading.attr="disabled" @click.stop="row == {{ $key }}; $wire.viewAssignment('{{ encrypt($batch->id) }}');" id="assignmentRowButton-{{ $key }}" aria-label="{{ __('View Assignment') }}" class="flex items-center justify-center z-0 p-1 outline-none rounded duration-200 ease-in-out" :class="{
                        ...
                        @else
                        <div class="relative bg-white px-4 pb-4 pt-2 h-[82.5vh] min-w-full flex items-center justify-center">
                    ...
                    <div class="relative w-full col-span-2">
                        ...
                        <div class="relative flex-col h-[18.5vh] flex bg-white p-2 rounded text-blue-1100 text-xs font-semibold">
                            ...
                            <div class="flex items-center justify-start ms-2 my-1">
                                Access Code: <p class="ps-2 font-normal">
                                    @if ($this->accessCode)
                                    <span class="bg-blue-300 text-blue-1000 rounded py-1 px-2 select-all">{{ $this->accessCode->access_code }}</span>
                                    @else
                                    <span class="font-semibold bg-amber-300 text-amber-950 rounded py-1 px-2 uppercase">not
                                        accessible</span>
                                    @endif
                                </p>
                            </div>
                        ...
                        <div class="flex items-center justify-end h-[1.5rem] my-2 w-full text-sm">
                            {{-- Found 3 special cases! Or Maybe Not --}}
                            <button type="button" wire:click="viewList" @if (!$batchId) disabled @endif class="flex items-center justify-center rounded px-3 py-1 text-sm font-bold duration-200 ease-in-out {{ $batchId ? 'bg-blue-700 hover:bg-blue-800 active:bg-blue-900 text-blue-50 hover:text-blue-100 active:text-blue-200 focus:ring-blue-500 focus:border-blue-500 focus:outline-blue-500' : 'bg-blue-300 text-blue-50' }}">
                                <p class="p-0 m-0">
                                    VIEW LIST
                                </p>
                        ...
                        {{-- Beneficiaries Table --}}
                        <div id="beneficiaries-table" class="relative h-[64vh] overflow-y-auto rounded whitespace-nowrap bg-white scrollbar-thin scrollbar-track-white scrollbar-thumb-blue-700">
                            <table class="w-full text-sm text-left text-blue-1100">
                                ...
                                <tbody class="text-xs">
                                    ...
                                    <tr class="border-b hover:bg-gray-50 whitespace-nowrap">
                                        <th scope="row" class="ps-3 py-2 font-semibold text-blue-1100 whitespace-nowrap">
                                            {{ $key + 1 }}
                                        </th>
                                        <td class="px-2 py-2">
                        ...
                        <div class="relative bg-white p-4 h-[64vh] min-w-full flex items-center justify-center">
                            <div class="relative flex flex-col items-center justify-center border rounded h-full w-full font-medium text-sm text-gray-500 bg-gray-50 border-gray-300">
                                ...
                                <p>Try <span class="underline underline-offset-2">clicking</span> one of the <span class="text-blue-900">batches</span> row.
                                </p>
    ...
    }" x-cloak x-show="successShow" x-transition:enter="transition ease-in-out duration-300 origin-left" x-transition:enter-start="opacity-0 scale-90" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="origin-left transition ease-in-out duration-500" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-90" class="fixed left-6 bottom-6 z-50 flex items-center bg-blue-200 text-blue-1000 border border-blue-500 rounded-lg text-sm sm:text-md font-bold px-4 py-3 select-none" role="alert">
