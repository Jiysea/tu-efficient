        ...
        <div class="p-2 min-h-screen select-none">
            ...
            <div class="relative grid grid-cols-1 w-full h-full gap-4 lg:grid-cols-5">
                ...
                <div class="relative lg:col-span-3 h-full w-full rounded bg-white shadow">
                    ...
                    <div class="relative max-h-12 flex items-center justify-between">
                        ...
                        <div class="mx-2 flex items-center justify-end">
                            <div class="relative me-2">
                                ...
                                <input type="text" id="project-search" maxlength="100" class="duration-200 ease-in-out ps-7 py-1 text-xs text-blue-1100 placeholder-blue-500 border border-blue-300 rounded w-full bg-blue-50 focus:ring-blue-500 focus:border-blue-500" placeholder="Search for project titles">
                            </div>
                            <button data-modal-target="create-modal" data-modal-toggle="create-modal" class="flex items-center bg-blue-900 hover:bg-blue-800 text-blue-50 hover:text-blue-100 rounded-md px-4 py-1 text-sm font-bold focus:ring-blue-500 focus:border-blue-500 focus:outline-blue-500 duration-200 ease-in-out">
                                FILTER
                    ...
                    <div id="batches-table" class="relative min-h-[84vh] max-h-[84vh] overflow-y-auto overflow-x-auto scrollbar-thin scrollbar-track-blue-50 scrollbar-thumb-blue-700">
                        <table class="relative w-full text-sm text-left text-blue-1100 whitespace-nowrap">
                            ...
                            <tbody class="relative text-xs">
                                ...
                                @endphp
                                <tr wire:key="batch-{{ $key }}" wire:click.prevent='selectBatchRow({{ $key }}, "{{ $encryptedId }}")' class="relative border-b bg-blue-50 hover:bg-blue-100 whitespace-nowrap duration-200 ease-in-out">
                                    <th scope="row" class="pe-2 ps-4 py-2 font-medium text-blue-1100">
                                        {{ $batch['batch_num'] }}
                                    </th>
                                    <td class="pr-6 py-2">
                                        {{ $batch['barangay_name'] }}
                                    ...
                                    <td class="pr-2 py-2 text-center">
                                        ...
                                        <span class="bg-green-300 text-green-1000 rounded py-1 px-2 uppercase font-medium">{{ $batch['approval_status'] }}</span>
                                        @else
                                        <span class="bg-amber-300 text-amber-950 rounded py-1 px-2 uppercase font-medium">{{ $batch['approval_status'] }}</span>
                                        @endif
                                    </td>
                                    <td class="pr-2 py-2 text-center">
                                        @if($batch['submission_status'] === 'unopened')
                                        <span class="bg-amber-300 text-amber-950 rounded py-1 px-2 uppercase font-medium">{{ $batch['submission_status'] }}</span>
                                        @elseif($batch['submission_status'] === 'encoding')
                                        <span class="bg-sky-300 text-sky-950 rounded py-1 px-2 uppercase font-medium">{{ $batch['submission_status'] }}</span>
                                        @elseif($batch['submission_status'] === 'submitted')
                                        <span class="bg-green-300 text-green-950 rounded py-1 px-2 uppercase font-medium">{{ $batch['submission_status'] }}</span>
                                        @elseif($batch['submission_status'] === 'revalidate')
                                        <span class="bg-red-300 text-red-950 rounded py-1 px-2 uppercase font-medium">{{ $batch['submission_status'] }}</span>
                                        @endif
                                    </td>

                                    {{-- Batch Dropdown --}}
                                    <td x-data="batchDropdown({{ $key }})" class="py-2 flex">
                                        <button @click.stop="handleClick()" id="batchRowButton-{{ $key }}" data-dropdown-placement="left" data-dropdown-toggle="batchRowDropdown-{{ $key }}" class="z-0 mx-1 p-1 font-medium rounded text-blue-1100 hover:text-blue-1000 active:text-blue-900 bg-transparent hover:bg-blue-200 duration-200 ease-in-out">
                    ...
                    <div wire:key="batchRowDropdown-{{ $key }}" id="batchRowDropdown-{{ $key }}" class="absolute z-50 hidden bg-white border rounded-md shadow">
                        <ul class="text-sm text-blue-1100" aria-labelledby="batchRowButton-{{ $key }}">
                            <li>
                                <a aria-label="{{ __('View Batch') }}" class="rounded-t-md flex items-center justify-start px-4 py-2 hover:text-blue-900 hover:bg-blue-100 duration-200 ease-in-out cursor-pointer">
                            ...
                            <li>
                                <a aria-label="{{ __('Access Code') }}" class="rounded-b-md flex items-center justify-start px-4 py-2 hover:text-blue-900 hover:bg-blue-100 duration-200 ease-in-out cursor-pointer">
                ...
                <div class="relative w-full col-span-2">
                    ...
                    <div class="relative bg-white p-2 rounded text-blue-1100 text-xs font-semibold">
                        {{-- Loading State --}}
                        <div class="absolute items-center top-0 left-0 justify-center z-50 min-h-full min-w-full text-blue-900" wire:loading.flex>
                    ...
                    <div class="flex items-center justify-end my-2 w-full text-sm">
                        ...
                        <button class="flex items-center bg-blue-900 text-blue-50 rounded-md px-3 py-1 text-sm font-bold focus:ring-blue-500 focus:border-blue-500 focus:outline-blue-500">
                    ...
                    <div class="relative min-h-[64vh] max-h-[64vh] overflow-y-auto rounded whitespace-nowrap bg-white">
                        {{-- Loading State --}}
                        <div class="absolute items-center justify-center z-50 min-h-full min-w-full w-full text-blue-900" wire:loading.flex>
                            <div class="absolute min-h-full min-w-full bg-black opacity-5">
                            </div>
                            <svg class="w-8 h-8 mr-3 -ml-1 animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4">
