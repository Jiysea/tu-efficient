...
<div x-data="{
    ...
    createProjectModal: $wire.entangle('createProjectModal'),
    viewProjectModal: $wire.entangle('viewProjectModal'),
    assignBatchesModal: $wire.entangle('assignBatchesModal'),
    viewBatchModal: $wire.entangle('viewBatchModal'),
    addBeneficiariesModal: $wire.entangle('addBeneficiariesModal'),
    viewBeneficiaryModal: $wire.entangle('viewBeneficiaryModal'),
...
window.addEventListener('resize', () => {
    ...
    <div :class="{
        ...
        <div class="p-2 min-h-screen select-none">
            ...
            <div class="relative flex items-center justify-between w-full gap-2 my-2 lg:my-0 lg:h-[7.5vh]">
                ...
                <template x-if="!isMobile">
                    <svg class="text-indigo-900 size-6 animate-spin" wire:loading
                        wire:target="calendarStart, calendarEnd, selectImplementationRow, viewImplementation, selectBatchRow, viewBatch, selectBeneficiaryRow, selectedBeneficiaryRow, selectShiftBeneficiary, viewBeneficiary, viewSignBeneficiary, loadMoreImplementations, loadMoreBeneficiaries, saveProject, editProject, deleteProject, saveBatches, editBatch, deleteBatch, saveBeneficiaries, editBeneficiary, deleteBeneficiary, archiveBeneficiary, removeBeneficiaries, showExport"
            ...
            <div class="relative grid grid-cols-1 w-full lg:h-[90vh] gap-4 lg:grid-cols-5">
                ...
                {{-- List of Batches --}}
                <div class="relative lg:col-span-2 size-full rounded bg-white shadow">
                    ...
                    <div class="relative flex justify-between max-h-12 items-center">
                        ...
                        <div class="relative mx-2 flex items-center">
                            @if ($implementationId)
                                ...
                                <div class="rounded-md py-1 px-2 text-xs me-2"
                                    :class="{
                                        'bg-amber-100 text-amber-700': {{ json_encode($this->remainingBatchSlots > 0 && $this->implementation?->status === 'pending') }},
                            ...
                            <span class="relative" x-data="{ pop: false }">
                                <button @mouseleave="pop = false;" @mouseenter="pop = true;"
                                    @if ($this->remainingBatchSlots === 0 || $this->implementation?->status !== 'pending') disabled 
                                @else
                                    @click="assignBatchesModal = !assignBatchesModal;" @endif
                                    class="flex items-center rounded-md px-3 py-1 text-sm font-bold duration-200 ease-in-out disabled:bg-gray-300 disabled:text-gray-500 disabled:cursor-not-allowed bg-indigo-900 hover:bg-indigo-800 text-indigo-50 hover:text-indigo-100 focus:ring-indigo-500 focus:border-indigo-500 focus:outline-indigo-500">
                                    ASSIGN
                                ...
                                <div x-cloak x-show="pop" x-transition.opacity
                                    class="absolute z-50 bottom-full mb-2 right-0 rounded p-2 shadow text-xs font-normal whitespace-nowrap border bg-zinc-900 border-zinc-300 text-indigo-50">
                                    @if ($implementationId && $this->remainingBatchSlots && $this->implementation?->status === 'pending')
                                        Assign Batches to Coordinators
                                    @elseif(!$implementationId)
                                        Please pick a <span class="text-indigo-400">project</span> first.
                                    @elseif($implementationId && $this->implementation?->status === 'implementing')
                                        You cannot <span class="text-indigo-400">assign</span> more batches to
                                        coordinators <br>
                                        when the project is currently implementing.
                                    @else
                                        You may able to <span class="text-indigo-400">assign</span> batches to
                                        coordinators <br>
                                        when there are still more unallocated slots.
                                    @endif
                                </div>
                            </span>
                        </div>
                        ...
                        <div class="relative px-2 flex flex-1 items-center justify-end gap-2">
                            ...
                            <span class="relative" x-data="{ pop: false }">
                                <button type="button" @mouseleave="pop = false;" @mouseenter="pop = true;"
                                    ...
                                    class="duration-200 ease-in-out flex items-center gap-2 justify-center px-2 py-1 rounded-md text-xs sm:text-sm font-bold outline-none disabled:bg-gray-300 disabled:text-gray-500 disabled:cursor-not-allowed text-indigo-50 bg-indigo-700 hover:bg-indigo-800 active:bg-indigo-900">
                            ...
                            <span class="relative" x-data="{ pop: false }">
                                ...
                                @elseif($this->implementation?->status === 'implementing')
                                    <button type="button" @mouseleave="pop = false;" @mouseenter="pop = true;"
                                        @if ($batchId) wire:click="viewSignBeneficiary"
                                        @click="$dispatch('contractSigned')"
                                    @else
                                    disabled @endif
                                        class="flex items-center gap-2 disabled:bg-gray-300 disabled:text-gray-500 disabled:cursor-not-allowed bg-red-700 hover:bg-red-800 active:bg-red-900 text-red-50 hover:text-red-100 focus:ring-red-500 focus:border-red-500 focus:outline-red-500 rounded-md px-2 py-1 text-sm font-bold duration-200 ease-in-out">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="size-4"
                                            xmlns:xlink="http://www.w3.org/1999/xlink" width="400" height="400"
                                            viewBox="0, 0, 400,400">
                                            <g>
    ...
    {{-- View Beneficiaries Modal --}}
    <livewire:focal.implementations.signing-beneficiaries-modal :$batchId :key="$batchId" />
                    ...
                    <div
                            ...
                            <div x-show="exportFormat === 'csv'" class="flex flex-col gap-2">
                                ...
                                <span class="flex items-center flex-wrap gap-2">
                                    ...
                                    <label for="annex_l_sign_csv"
                                        ...
                                        Annex L (Payroll w/ Sign)
                                        <input type="radio" class="hidden absolute inset-0" id="annex_l_sign_csv"
                                            value="annex_l_sign" wire:model.live="exportTypeCsv">
                                    </label>
                                </span>
                            </div>
                        ...
                        <div x-data="exporting" class="relative w-full flex items-center gap-2">
                            ...
                            <label for="xlsx-radio"
                                ...
                                XLSX
                                <input type="radio" class="hidden absolute inset-0" id="xlsx-radio" value="xlsx"
                            ...
                            <label for="csv-radio"
                                ...
                                CSV
                                <input type="radio" class="hidden absolute inset-0" id="csv-radio" value="csv"
                                    wire:model.live="exportFormat">
                            </label>
                        </div>
                                            ...
                                            @else
                                                <div
                                                    ...
                                                    @else
                                                        ...
                                                        <p>No batches found.</p>
                                                        <p>Try assigning a <span class="text-indigo-900">
                                                                a new batch</span>.
                                                        </p>
                                                    @endif
                                                </div>
