...
window.matchMedia('(min-width: 1280px)').addEventListener('change', event => {
                ...
                <div class="relative flex items-center my-2">
                    ...
                    <div id="implementations-date-range" date-rangepicker datepicker-autohide class="flex items-center">
                        ...
                        <div class="relative">
                            ...
                            <input id="start-date" name="start" type="text" value="{{ $defaultStart }}" datepicker-max-date="{{ $defaultStart }}" class="bg-white border border-indigo-300 text-indigo-1100 text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block w-full ps-10" placeholder="Select date start">
                        </div>
                        ...
                        <div class="relative">
                            ...
                            <input id="end-date" name="end" type="text" value="{{ $defaultEnd }}" datepicker-min-date="{{ $defaultEnd }}" class="bg-white border border-indigo-300 text-indigo-1100 text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block w-full ps-10" placeholder="Select date end">
                        </div>
                    </div>
                    ...
                    <div x-data="{ createProjectModal: $wire.entangle('createProjectModal'), viewProjectModal: $wire.entangle('viewProjectModal') }" class="relative lg:col-span-3 h-full w-full rounded bg-white shadow">
                        ...
                        <div class="relative max-h-12 flex items-center justify-between">
                            <div class="inline-flex my-2 items-center text-indigo-900">
                                ...
                                <span class="{{ $totalImplementations ? 'bg-indigo-100 text-indigo-700' : 'bg-red-100 text-red-700 ' }} rounded ms-2 px-2 py-1 font-medium text-xs">{{ $totalImplementations ?? 0 }}</span>
                            </div>
                            ...
                            <div class="mx-2 flex items-center justify-end">
                                ...
                                <div class="relative me-2">
                                    ...
                                    <div class="absolute inset-y-0 start-0 flex items-center ps-2 pointer-events-none {{ $this->implementations->isNotEmpty() || $searchProjects ? 'text-indigo-800' : 'text-zinc-400' }}">

                                        {{-- Loading Icon --}}
                                        <svg class="size-3 animate-spin" wire:loading wire:target="searchProjects" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4">
                                            </circle>
                                            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z">
                                            </path>
                                        ...
                                        {{-- Search Icon --}}
                                        <svg class="size-3" wire:loading.remove wire:target="searchProjects" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 20">
                                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m19 19-4-4m0-7A7 7 0 1 1 1 8a7 7 0 0 1 14 0Z" />
                                        </svg>
                                    </div>

                                    {{-- Search Input Bar --}}
                                    <input type="text" id="project-search" maxlength="100" autocomplete="off" @if ($this->implementations->isEmpty() && !$searchProjects) disabled @endif
                                ...
                                <button @click="createProjectModal = !createProjectModal;" class="flex items-center bg-indigo-900 hover:bg-indigo-800 text-indigo-50 hover:text-indigo-100 rounded-md px-4 py-1 text-sm font-bold focus:ring-indigo-500 focus:border-indigo-500 focus:outline-indigo-500 duration-200 ease-in-out">
                                    CREATE
                        ...
                        <div id="implementations-table" class="relative h-[36vh] overflow-y-auto overflow-x-auto scrollbar-thin scrollbar-track-indigo-50 scrollbar-thumb-indigo-700">
                            <table class="relative w-full text-sm text-left text-indigo-1100 whitespace-nowrap">
                                ...
                                <tbody class="relative text-xs">
                                    ...
                                    <tr wire:key="implementation-{{ $key }}" wire:loading.class="pointer-events-none" wire:target="selectImplementationRow" wire:click.prevent='selectImplementationRow({{ $key }}, "{{ encrypt($implementation->id) }}")' class="relative border-b duration-200 ease-in-out {{ $selectedImplementationRow === $key ? 'bg-gray-200 text-indigo-900 hover:bg-gray-300' : ' hover:bg-gray-50' }} whitespace-nowrap cursor-pointer">
                        ...
                        @else
                        <div class="relative bg-white px-4 pb-4 pt-2 h-[36vh] min-w-full flex items-center justify-center">
                    ...
                    <div x-data="{ assignBatchesModal: $wire.entangle('assignBatchesModal'), viewBatchModal: $wire.entangle('viewBatchModal') }" class="relative lg:col-span-2 h-full w-full rounded bg-white shadow">
                        ...
                        <div class="relative flex justify-between max-h-12 items-center">
                            ...
                            <div class="mx-2 flex items-center">
                                ...
                                <div class="{{ $remainingBatchSlots > 0 ? 'bg-amber-100 text-amber-700' : 'bg-green-100 text-green-700' }} rounded-md py-1 px-2 text-xs me-2">
                                    {{ $remainingBatchSlots }}</div>
                                @endif
                                <button @if (!$remainingBatchSlots || $remainingBatchSlots===null || $remainingBatchSlots===0) disabled @endif @if ($implementationId) @click="assignBatchesModal = !assignBatchesModal;" @endif class="flex items-center rounded-md px-3 py-1 text-sm font-bold duration-200 ease-in-out {{ $remainingBatchSlots > 0 ? 'bg-indigo-900 hover:bg-indigo-800 text-indigo-50 hover:text-indigo-100 focus:ring-indigo-500 focus:border-indigo-500 focus:outline-indigo-500' : 'bg-indigo-300 text-indigo-50' }}">
                                    ASSIGN
                        ...
                        {{-- Batches Table --}}
                        <div id="batches-table" class="relative h-[36vh] overflow-y-auto scrollbar-thin scrollbar-track-indigo-50 scrollbar-thumb-indigo-700">
                            ...
                            <table class="relative w-full text-sm text-left text-indigo-1100">
                                ...
                                <tbody class="text-xs relative">
                                    ...
                                    <tr wire:key="batch-{{ $key }}" wire:loading.class="pointer-events-none" wire:target="selectBatchRow" wire:click='selectBatchRow({{ $key }}, "{{ encrypt($batch->id) }}")' class="relative border-b whitespace-nowrap duration-200 ease-in-out cursor-pointer {{ $selectedBatchRow === $key ? 'bg-gray-100 text-indigo-900 hover:bg-gray-200' : ' hover:bg-gray-50' }}">
                                        <td class="absolute h-full w-1 left-0 z-50" :class="{
                                        ...
                                        <td class="py-2 text-center">
                                            <span class="px-3 py-1 text-xs rounded-full font-semibold uppercase {{ $batch->approval_status === 'approved' ? 'bg-green-300 text-green-1000' : 'bg-amber-300 text-amber-900' }}">
                                                {{ $batch->approval_status }}
                                            </span>
                                        </td>
                                        <td class="py-1 ps-2">
                                            <button @click.stop="$wire.viewBatch('{{ encrypt($batch->id) }}');" id="batchRowButton-{{ $key }}" class="flex justify-center items-center z-0 p-1 font-medium rounded outline-none duration-200 ease-in-out {{ $selectedBatchRow === $key ? 'hover:bg-indigo-700 focus:bg-indigo-700 text-indigo-900 hover:text-indigo-50 focus:text-indigo-50' : 'text-gray-900 hover:text-indigo-900 focus:text-indigo-900 hover:bg-gray-300 focus:bg-gray-300' }}">
                        ...
                        <div class="relative bg-white px-4 pb-4 pt-2 h-[36vh] min-w-full flex items-center justify-center">
                            <div class="relative flex flex-col items-center justify-center border rounded h-full w-full font-medium text-sm text-gray-500 bg-gray-50 border-gray-300">
                                ...
                                <p>Try <span class="underline underline-offset-2">clicking</span> one of the <span class=" text-indigo-900">projects</span> row.
                                </p>
                                @else
                                <svg xmlns="http://www.w3.org/2000/svg" class=" size-12 sm:size-20 mb-4 text-indigo-900 opacity-65" xmlns:xlink="http://www.w3.org/1999/xlink" width="400" height="400" viewBox="0, 0, 400,400">
                                    <g>
                    ...
                    <div x-data="{ addBeneficiariesModal: $wire.entangle('addBeneficiariesModal'), viewBeneficiaryModal: $wire.entangle('viewBeneficiaryModal'), importFileModal: $wire.entangle('importFileModal') }" class="relative lg:col-span-5 h-full w-full rounded bg-white shadow">
                        ...
                        <div class="relative max-h-12 items-center grid row-span-1 grid-cols-2">
                            <div class="inline-flex items-center my-2 col-span-1 text-indigo-900">
                                ...
                                <span class="{{ $batchId ? 'bg-indigo-100 text-indigo-700' : 'bg-red-100 text-red-700' }} rounded px-2 py-1 ms-2 text-xs font-medium">
                            ...
                            <div class="col-span-1 mx-2 flex items-center justify-end">
                                ...
                                {{-- Search Box --}}
                                <div class="relative me-2">
                                    <div class="absolute inset-y-0 start-0 flex items-center ps-2 pointer-events-none {{ $beneficiarySlots['num_of_beneficiaries'] ? 'text-indigo-800' : 'text-zinc-400' }}">

                                        {{-- Loading Icon --}}
                                        <svg class="size-4 animate-spin" wire:loading wire:target="searchBeneficiaries" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4">
                                            </circle>
                                            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z">
                                            </path>
                                    ...
                                    <input type="text" id="beneficiary-search" maxlength="100" autocomplete="off" @if (!$beneficiarySlots['num_of_beneficiaries']) disabled @endif @input.debounce.300ms="$wire.searchBeneficiaries = $el.value; $wire.$refresh();" class="{{ $beneficiarySlots['num_of_beneficiaries']
                                        ? 'text-indigo-1100 placeholder-indigo-500 border-indigo-300 bg-indigo-50 focus:ring-indigo-500 focus:border-indigo-500'
                                        : 'text-zinc-400 placeholder-zinc-400 border-zinc-300 bg-zinc-50' }} duration-200 ease-in-out ps-7 py-1 text-xs border rounded w-full " placeholder="Search for beneficiaries">
                                </div>

                                <button @if ($batchId && $beneficiarySlots['batch_slots_allocated']> $beneficiarySlots['num_of_beneficiaries']) @click="importFileModal = !importFileModal;" @else disabled @endif
                                    ...
                                    IMPORT
                                    <svg xmlns="http://www.w3.org/2000/svg" class="size-4 ml-2" xmlns:xlink="http://www.w3.org/1999/xlink" width="400" height="400" viewBox="0, 0, 400,400">
                                        <g>
                                ...
                                <button @if ($batchId && $beneficiarySlots['batch_slots_allocated']> $beneficiarySlots['num_of_beneficiaries']) @click="addBeneficiariesModal = !addBeneficiariesModal;" @else disabled @endif
                        ...
                        {{-- Beneficiaries Table --}}
                        <div id="beneficiaries-table" class="relative h-[38.5vh] overflow-y-auto overflow-x-auto scrollbar-thin scrollbar-track-indigo-50 scrollbar-thumb-indigo-700">
                            <table class="relative w-full text-sm text-left text-indigo-1100">
                                ...
                                <tbody class="text-xs">
                                    ...
                                    <tr wire:key="beneficiary-{{ $key }}" {{-- wire:loading.class="pointer-events-none"
                                            wire:target="selectBeneficiaryRow" --}} @click="$wire.selectBeneficiaryRow({{ $key }}, '{{ encrypt($beneficiary->id) }}');" @dblClick="$wire.viewBeneficiary('{{ encrypt($beneficiary->id) }}');" class="relative border-b divide-x whitespace-nowrap cursor-pointer" :class="{
                                        ...
                                        <td class="py-1">
                                            <button type="button" @click.stop="$wire.viewBeneficiary('{{ encrypt($beneficiary->id) }}');" id="beneficiaryRowButton-{{ $key }}" class="flex items-center justify-center z-0 mx-1 p-1 font-medium rounded outline-none duration-200 ease-in-out" :class="{
                        ...
                        @else
                        <div class="relative bg-white px-4 pb-4 pt-2 h-[38.5vh] min-w-full flex items-center justify-center">
                            <div class="relative flex flex-col items-center justify-center border rounded h-full w-full font-medium text-sm text-gray-500 bg-gray-50 border-gray-300">
                                @if ($this->implementations->isEmpty())
                                <svg xmlns="http://www.w3.org/2000/svg" class="size-12 sm:size-20 mb-4 text-indigo-900 opacity-65" xmlns:xlink="http://www.w3.org/1999/xlink" width="400" height="400" viewBox="0, 0, 400,400">
                                    <g>
                                ...
                                <p>Try <span class="underline underline-offset-2">clicking</span> one of the <span class=" text-indigo-900">batches</span> row.
                                </p>
                                @else
                                <svg xmlns="http://www.w3.org/2000/svg" class="size-12 sm:size-20 mb-4 text-indigo-900 opacity-65" xmlns:xlink="http://www.w3.org/1999/xlink" width="400" height="400" viewBox="0, 0, 400,400">
                                    <g>
    ...
    }" x-cloak x-show="successShow" x-transition:enter="transition ease-in-out duration-300 origin-left" x-transition:enter-start="opacity-0 scale-90" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="origin-left transition ease-in-out duration-500" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-90" class="fixed left-6 bottom-6 z-50 flex items-center bg-indigo-200 text-indigo-1000 border border-indigo-500 rounded-lg text-sm sm:text-md font-bold px-4 py-3 select-none" role="alert">
