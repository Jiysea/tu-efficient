...
window.matchMedia('(min-width: 1280px)').addEventListener('change', event => {
            ...
            <div class="p-2 min-h-screen select-none">
                    ...
                    <div id="implementations-date-range" date-rangepicker datepicker-autohide class="flex items-center">
                        <div class="relative">
                            <div class="absolute inset-y-0 start-0 flex items-center ps-3 pointer-events-none">
                                <svg class="w-4 h-4 text-indigo-900 " aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                                    <path d="M20 4a2 2 0 0 0-2-2h-2V1a1 1 0 0 0-2 0v1h-3V1a1 1 0 0 0-2 0v1H6V1a1 1 0 0 0-2 0v1H2a2 2 0 0 0-2 2v2h20V4ZM0 18a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V8H0v10Zm5-8h10a1 1 0 0 1 0 2H5a1 1 0 0 1 0-2Z" />
                                </svg>
                            </div>
                            <input id="start-date" name="start" type="text" value="{{ $defaultStart }}" class="bg-white border border-indigo-300 text-indigo-1100 text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block w-full ps-10" placeholder="Select date start">
                        </div>
                        ...
                        <div class="relative">
                            ...
                            <input id="end-date" name="end" type="text" value="{{ $defaultEnd }}" class="bg-white border border-indigo-300 text-indigo-1100 text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block w-full ps-10" placeholder="Select date end">
                        </div>
                    </div>

                    {{-- Loading State --}}
                    <div class="absolute items-center justify-end z-50 min-h-full min-w-full text-indigo-900" wire:loading.flex>
                ...
                <div class="relative grid grid-cols-1 w-full h-full gap-4 lg:grid-cols-5">
                    ...
                    <div class="relative lg:col-span-3 h-full w-full rounded bg-white shadow">
                        ...
                        <div id="implementations-table" class="relative min-h-60 max-h-60 overflow-y-auto overflow-x-auto scrollbar-thin scrollbar-track-indigo-50 scrollbar-thumb-indigo-700">
                            <table class="relative w-full text-sm text-left text-indigo-1100 whitespace-nowrap">
                                ...
                                <tbody class="relative text-xs">
                                    ...
                                    @endphp
                                    <tr @if ($beneficiaries) @click="scrollToTop()" @endif wire:key="implementation-{{ $key }}" wire:click.prevent='selectImplementationRow({{ $key }}, "{{ $encryptedId }}")' class="relative border-b {{ $selectedImplementationRow === $key ? 'bg-indigo-100 hover:bg-indigo-200' : 'bg-white hover:bg-indigo-50' }}  whitespace-nowrap duration-200 ease-in-out">
                                        ...
                                        <td x-data="iDropdownRotation({{ $key }})" class="py-2 flex">
                                            <button @click.stop="handleClick()" id="implementationRowButton-{{ $key }}" data-dropdown-placement="left" data-dropdown-toggle="implementationRowDropdown-{{ $key }}" class="z-0 mx-1 p-1 font-medium rounded text-indigo-1100 hover:text-indigo-1000 active:text-indigo-900 bg-transparent hover:bg-indigo-200 duration-200 ease-in-out">
                        ...
                        <div class="relative bg-white px-4 pb-4 pt-2 h-60 min-w-full flex items-center justify-center">
                            <div class="relative flex flex-col items-center justify-center border rounded h-full w-full font-medium text-sm text-gray-500 bg-gray-50 border-gray-300">
                                <svg xmlns="http://www.w3.org/2000/svg" class="animate-pulse size-12 sm:size-20 mb-4 text-gray-300" xmlns:xlink="http://www.w3.org/1999/xlink" width="400" height="400" viewBox="0, 0, 400,400">
                                    <g>
                    ...
                    <div class="relative lg:col-span-2 h-full w-full rounded bg-white shadow">
                        ...
                        @if ($batches)
                        {{-- Table --}}
                        <div id="batches-table" class="relative min-h-60 max-h-60 overflow-y-auto scrollbar-thin scrollbar-track-indigo-50 scrollbar-thumb-indigo-700">
                            ...
                            <table class="relative w-full text-sm text-left text-indigo-1100">
                                ...
                                <tbody class="text-xs relative">
                                    ...
                                    @endphp
                                    <tr @if ($beneficiaries) @click="scrollToTop()" @endif wire:click='selectBatchRow({{ $key }}, "{{ $batchEncryptedId }}")' wire:key='batch-{{ $key }}' class="relative border-b {{ $selectedBatchRow === $key ? 'bg-indigo-100 hover:bg-indigo-200' : 'bg-white hover:bg-indigo-50' }} whitespace-nowrap duration-200 ease-in-out">
                                        ...
                                        <td class="py-2">
                                            <p class="px-1 py-1 text-xs rounded font-semibold uppercase {{ $batch['approval_status'] === 'approved' ? 'bg-green-300 text-green-1000' : 'bg-gray-500 text-gray-50' }}  text-center">
                                                {{ $batch['approval_status'] }}
                                            </p>
                                        </td>
                                        <td x-data="bDropdownRotation({{ $key }})" class="py-2 flex">
                                            <button @click.stop="handleClick()" id="batchRowButton-{{ $key }}" data-dropdown-placement="left" data-dropdown-toggle="batchRowDropdown-{{ $key }}" class="z-0 mx-1 p-1 font-medium rounded text-indigo-1100 hover:text-indigo-1000 active:text-indigo-900 bg-transparent hover:bg-indigo-200 duration-200 ease-in-out">
                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" :class="{
                                                            'rotate-0': !isVisible(),
                                                            'rotate-90': isVisible(),
                                                        }" class="w-4 duration-300 ease-in-out">
                    ...
                    <div class="relative lg:col-span-5 h-full w-full rounded bg-white shadow">
                        ...
                        <div class="relative max-h-12 items-center grid row-span-1 grid-cols-2">
                            ...
                            <div class="col-span-1 mx-2 flex items-center justify-end">
                                ...
                                <div class="relative me-2">
                                    <div class="absolute inset-y-0 start-0 flex items-center ps-2 pointer-events-none">
                                        <svg class="size-3 text-indigo-800" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 20">
                                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m19 19-4-4m0-7A7 7 0 1 1 1 8a7 7 0 0 1 14 0Z" />
                                        </svg>
                                    </div>
                                    <input type="text" id="beneficiary-search" maxlength="100" {{-- wire:model.live="searchBeneficiaries" --}} class="duration-200 ease-in-out ps-7 py-1 text-xs text-indigo-1100 placeholder-indigo-500 border border-indigo-300 rounded w-full bg-indigo-50 focus:ring-indigo-500 focus:border-indigo-500" placeholder="Search for beneficiaries">
                                </div>

                                <button @if ($batches && $beneficiarySlots['batch_slots_allocated']> $beneficiarySlots['num_of_beneficiaries']) data-modal-target="add-beneficiaries-modal" data-modal-toggle="add-beneficiaries-modal" @else disabled @endif
                                    ...
                                    ADD
                                    <svg class="size-4 ml-2" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="400" height="400" viewBox="0, 0, 400,400">
                                        <g>
                        ...
                        {{-- Beneficiaries Table --}}
                        <div id="beneficiary-table" class="relative max-h-60 overflow-y-auto overflow-x-auto scrollbar-thin scrollbar-track-indigo-50 scrollbar-thumb-indigo-700">
                            <table class="relative w-full text-sm text-left text-indigo-1100">
                                ...
                                <tbody class="text-xs">
                                    ...
                                    @endphp
                                    <tr wire:click.prevent="selectBeneficiaryRow({{ $key }}, '{{ $encryptedId }}')" wire:key="beneficiary-{{ $key }}" class="relative {{ $selectedBeneficiaryRow === $key ? 'bg-indigo-100 hover:bg-indigo-200' : 'bg-white hover:bg-indigo-50' }} border-b whitespace-nowrap">
                                        <th scope="row" class="pe-2 border-r border-gray-200 ps-4 py-2 font-medium text-indigo-1100 whitespace-nowrap ">
                                            {{ $key + 1 }}
                                        </th>
                                        ...
                                        <td x-data="bfDropdownRotation({{ $key }})" class="py-2 flex">
                                            <button @click.stop="handleClick()" id="beneficiaryRowButton-{{ $key }}" data-dropdown-placement="left" data-dropdown-toggle="beneficiaryRowDropdown-{{ $key }}" class="z-0 mx-1 p-1 font-medium rounded text-indigo-1100 hover:text-indigo-1000 focus:outline-none active:text-indigo-900 bg-transparent hover:bg-indigo-200 duration-200 ease-in-out">
                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" :class="{
                                                            'rotate-0': !isVisible(),
                                                            'rotate-90': isVisible(),
                                                        }" class="w-4 duration-300 ease-in-out">
    ...
    }" x-cloak x-show="successShow" {{-- x-effect="console.log(successShow);" --}} x-transition:enter="transition ease-in-out duration-300 origin-left" x-transition:enter-start="opacity-0 scale-90" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="origin-left transition ease-in-out duration-500" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-90" class="fixed left-6 bottom-6 z-50 flex items-center bg-indigo-200 text-indigo-1000 border border-indigo-500 rounded-lg text-sm sm:text-md font-bold px-4 py-3 select-none" role="alert">
    ...
    <script>
        ...
        Alpine.data('iDropdownRotation', (key) => ({
            ...
            init() {
                ...
                this.dropdown = new Dropdown(
                    document.getElementById(`implementationRowDropdown-${key}`)
                    , document.getElementById(`implementationRowButton-${key}`)
                );
            }
            , handleClick() {
        ...
        Alpine.data('bDropdownRotation', (key) => ({
            ...
            init() {
                // Initialize the dropdown when the Alpine component is initialized
                this.dropdown = new Dropdown(
                    document.getElementById(`batchRowDropdown-${key}`)
                    , document.getElementById(`batchRowButton-${key}`)
                );
            }
            , handleClick() {
        ...
        Alpine.data('bfDropdownRotation', (key) => ({
            ...
            init() {
                // Initialize the dropdown when the Alpine component is initialized
                this.dropdown = new Dropdown(
                    document.getElementById(`beneficiaryRowDropdown-${key}`)
                    , document.getElementById(`beneficiaryRowButton-${key}`)
                );
            }
            , handleClick() {
