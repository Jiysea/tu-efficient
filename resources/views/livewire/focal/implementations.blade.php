            ...
            <div class="relative grid grid-cols-1 w-full h-full gap-4 lg:grid-cols-5">
                            ...
                            <table class="relative w-full text-sm text-left text-indigo-1100">
                                ...
                                <tbody class="text-xs relative">
                                    @foreach ($this->batches as $key => $batch)
                                        <tr wire:key="batch-{{ $key }}" @dblclick="$wire.viewBatch('{{ encrypt($batch->id) }}')"
                ...
                <div x-data="{ addBeneficiariesModal: $wire.entangle('addBeneficiariesModal'), viewBeneficiaryModal: $wire.entangle('viewBeneficiaryModal'), importFileModal: $wire.entangle('importFileModal'), showExportModal: $wire.entangle('showExportModal') }" class="relative lg:col-span-5 h-full w-full rounded bg-white shadow">
                    ...
                    <div class="relative max-h-12 items-center grid row-span-1 grid-cols-2">
                        ...
                        {{-- Search and Add Button | and Slots (for lower lg) --}}
                        <div class="col-span-1 mx-2 flex items-center justify-end gap-2">
                            ...
                            {{-- Export Button --}}
                            <button type="button" @if ($this->exportBatches->isNotEmpty() && $selectedBatchRow !== -1) @click="showExportModal = !showExportModal;" @else disabled @endif
                                ...
                                <div class="relative w-full bg-white rounded-md shadow">
                                    ...
                                    <div class="flex items-center justify-between py-2 px-4 rounded-t-md">
                                        ...
                                        <div class="flex items-center justify-end gap-2">
                                            ...
                                            {{-- Loading State --}}
                                            <svg xmlns="http://www.w3.org/2000/svg" class="size-6 text-indigo-900 animate-spin" wire:loading wire:target="exportAnnex, showExport, exportFormat, defaultExportStart, defaultExportEnd, selectExportBatchRow" fill="none" viewBox="0 0 24 24">
                                            ...
                                            {{-- Close Button --}}
                                            <button type="button" @click="$wire.resetExport(); showExportModal = false;" class="outline-none text-indigo-400 hover:bg-indigo-200 hover:text-indigo-900 rounded  size-8 ms-auto inline-flex justify-center items-center duration-300 ease-in-out">
                                    ...
                                    {{-- Modal body --}}
                                    <div class="w-full flex flex-col items-center justify-center gap-4 pt-5 pb-6 px-3 md:px-12 text-indigo-1100 text-xs">
                                        ...
                                        <div class="relative w-full flex items-center gap-2">
                                            ...
                                            <div class="flex flex-col gap-2">
                                                <span class="flex items-center flex-wrap gap-2">
                                                    {{-- Annex E-1 (COS) --}}
                                                    <label for="annex_e1" class="relative duration-200 ease-in-out cursor-pointer whitespace-nowrap flex items-center justify-center px-3 py-2 rounded font-semibold {{ $exportType['annex_e1'] ? 'bg-indigo-700 text-indigo-50' : 'bg-gray-200 text-gray-500' }}">
                                                        Annex E-1 (COS)
                                                    ...
                                                    <label for="annex_e2" class="relative duration-200 ease-in-out cursor-pointer whitespace-nowrap flex items-center justify-center px-3 py-2 rounded font-semibold {{ $exportType['annex_e2'] ? 'bg-indigo-700 text-indigo-50' : 'bg-gray-200 text-gray-500' }}">
                                                        Annex E-2 (COS - Co-Partner)
                                                    ...
                                                    <label for="annex_j2" class="relative duration-200 ease-in-out cursor-pointer whitespace-nowrap flex items-center justify-center px-3 py-2 rounded font-semibold {{ $exportType['annex_j2'] ? 'bg-indigo-700 text-indigo-50' : 'bg-gray-200 text-gray-500' }}">
                                                        Annex J-2 (Attendance Sheet)
                                                ...
                                                <span class="flex items-center flex-wrap gap-2">
                                                    {{-- Annex L (Payroll) --}}
                                                    <label for="annex_l" class="relative duration-200 ease-in-out cursor-pointer whitespace-nowrap flex items-center justify-center px-3 py-2 rounded font-semibold {{ $exportType['annex_l'] ? 'bg-indigo-700 text-indigo-50' : 'bg-gray-200 text-gray-500' }}">
                                                        Annex L (Payroll)
                                                        <input type="checkbox" class="hidden absolute inset-0" id="annex_l" wire:model.live="exportType.annex_l">
                                                    </label>
                                                    {{-- Annex L (Payroll w/ Sign) --}}
                                                    <label for="annex_l_sign" class="relative duration-200 ease-in-out cursor-pointer whitespace-nowrap flex items-center justify-center px-3 py-2 rounded font-semibold {{ $exportType['annex_l_sign'] ? 'bg-indigo-700 text-indigo-50' : 'bg-gray-200 text-gray-500' }}">
                                                        Annex L (Payroll w/ Sign)
                                        ...
                                        <div class="relative w-full flex items-center gap-2">
                                            ...
                                            {{-- XLSX --}}
                                            <label for="xlsx-radio" class="relative duration-200 ease-in-out cursor-pointer whitespace-nowrap flex items-center justify-center px-3 py-2 rounded font-semibold {{ $exportFormat === 'xlsx' ? 'bg-indigo-700 text-indigo-50' : 'bg-gray-200 text-gray-500' }}">
                                                XLSX
                                                <input type="radio" class="hidden absolute inset-0" id="xlsx-radio" value="xlsx" wire:model.live="exportFormat">
                                            </label>
                                            {{-- CSV --}}
                                            <label for="csv-radio" class="relative duration-200 ease-in-out cursor-not-allowed whitespace-nowrap flex items-center justify-center px-3 py-2 rounded font-semibold {{ $exportFormat === 'csv' ? 'bg-indigo-700 text-indigo-50' : 'bg-gray-50 text-gray-300' }}">
                                                CSV
                                        ...
                                        <div class="w-full flex flex-col justify-center gap-4">
                                            {{-- Date Range --}}
                                            <div id="export-date-range" datepicker-orientation="top" date-rangepicker datepicker-autohide class="flex items-center gap-1 sm:gap-2 pb-4 text-xs text-indigo-1100">

                                                {{-- Start --}}
                                                <span class="text-sm font-medium">Filter Date:</span>
                                                <div class="relative">
                                                    <span class="absolute {{ $errors->has('defaultExportStart') ? 'text-red-900' : 'text-indigo-900 ' }} inset-y-0 start-0 flex items-center ps-2 pointer-events-none">
                                                ...
                                                {{-- End --}}
                                                <div class="relative">
                                                    <span class="absolute {{ $errors->has('defaultExportEnd') ? 'text-red-900' : 'text-indigo-900 ' }} inset-y-0 start-0 flex items-center ps-2 pointer-events-none">
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="size-3.5 sm:size-5" xmlns:xlink="http://www.w3.org/1999/xlink" width="400" height="400" viewBox="0, 0, 400,400">
                                                            <g>
                                                                <path d="M126.172 51.100 C 118.773 54.379,116.446 59.627,116.423 73.084 L 116.406 83.277 108.377 84.175 C 76.942 87.687,54.343 110.299,50.788 141.797 C 49.249 155.427,50.152 292.689,51.825 299.512 C 57.852 324.094,76.839 342.796,101.297 348.245 C 110.697 350.339,289.303 350.339,298.703 348.245 C 323.161 342.796,342.148 324.094,348.175 299.512 C 349.833 292.748,350.753 155.358,349.228 142.055 C 345.573 110.146,323.241 87.708,291.623 84.175 L 283.594 83.277 283.594 73.042 C 283.594 56.745,279.386 50.721,267.587 50.126 C 254.712 49.475,250.000 55.397,250.000 72.227 L 250.000 82.813 200.000 82.813 L 150.000 82.813 150.000 72.227 C 150.000 58.930,148.409 55.162,141.242 51.486 C 137.800 49.721,129.749 49.515,126.172 51.100 M293.164 118.956 C 308.764 123.597,314.804 133.574,316.096 156.836 L 316.628 166.406 200.000 166.406 L 83.372 166.406 83.904 156.836 C 85.337 131.034,93.049 120.612,112.635 118.012 C 123.190 116.612,288.182 117.474,293.164 118.956 M316.400 237.305 C 316.390 292.595,315.764 296.879,306.321 306.321 C 296.160 316.483,296.978 316.405,200.000 316.405 C 103.022 316.405,103.840 316.483,93.679 306.321 C 84.236 296.879,83.610 292.595,83.600 237.305 L 83.594 200.000 200.000 200.000 L 316.406 200.000 316.400 237.305 " stroke="none" fill="currentColor" fill-rule="evenodd">
                                                                </path>
                                            ...
                                            <div class="flex items-center w-full gap-2">
                                                ...
                                                <div x-data="{ show: false, currentBatch: $wire.entangle('currentExportBatch') }" class="relative z-30">
                                                    ...
                                                    {{-- Content --}}
                                                    <div x-show="show" @click.away="show = false; $wire.set('searchExportBatch', null);" class="absolute -left-20 sm:inset-x-0 bottom-full p-3 mb-2 w-96 text-indigo-1100 bg-white shadow-lg border border-indigo-100 rounded text-xs">

                                                        {{-- Batches Count | Search Bar --}}
                                                        <div class="flex items-center w-full gap-2">
                                                            {{-- Batches Count --}}
                                                            <span class="flex items-center gap-2 rounded {{ $this->exportBatches->isNotEmpty() ? 'text-indigo-900 bg-indigo-100' : 'text-red-900 bg-red-100' }} py-1.5 px-2 text-xs select-none">
                                                                <svg xmlns="http://www.w3.org/2000/svg" class="size-4" xmlns:xlink="http://www.w3.org/1999/xlink" width="400" height="400" viewBox="0, 0, 400,400">
                                                                    <g>
                                                            ...
                                                            {{-- Search Bar --}}
                                                            <div class="relative flex flex-1 items-center justify-center py-1 text-indigo-700">
                                                                ...
                                                                {{-- Search Bar --}}
                                                                <input id="searchExportBatch" wire:model.live.debounce.350ms="searchExportBatch" type="text" autocomplete="off" class="rounded w-full ps-8 py-1.5 text-xs text-indigo-1100 border-indigo-200 hover:placeholder-indigo-500 hover:border-indigo-500 focus:border-indigo-900 focus:ring-1 focus:ring-indigo-900 focus:outline-none duration-200 ease-in-out" placeholder="Search batch number">
                                                            </div>
                                                        </div>
                                                        <ul class="mt-2 text-xs overflow-y-auto h-40 scrollbar-thin scrollbar-track-indigo-50 scrollbar-thumb-indigo-700">
                                                            ...
                                                            <div class="flex flex-1 items-center justify-center size-full text-sm border border-gray-300 bg-gray-100 text-gray-500 rounded p-2">
                                                                ...
                                                                <svg xmlns="http://www.w3.org/2000/svg" class="size-12 mb-4 text-indigo-900 opacity-65" xmlns:xlink="http://www.w3.org/1999/xlink" width="400" height="400" viewBox="0, 0, 400,400">
                                                                    <g>
                                                                ...
                                                                <p>Maybe try a different <span class=" text-indigo-900">search
                                                                        term</span>?
                                                                </p>
                                                                ...
                                                                <p>No batches found.</p>
                                                                <p>Try adjusting the <span class=" text-indigo-900">filter
                                                                        date</span>.
                                                                </p>
                                                                @else
                                                                <svg xmlns="http://www.w3.org/2000/svg" class="size-12 mb-4 text-indigo-900 opacity-65" xmlns:xlink="http://www.w3.org/1999/xlink" width="400" height="400" viewBox="0, 0, 400,400">
                                                                ...
                                                                <p>No batches found.</p>
                                                                <p>Ask your focal to <span class=" text-indigo-900">assign
