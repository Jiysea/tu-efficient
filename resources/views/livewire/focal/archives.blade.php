...
window.matchMedia('(min-width: 1280px)').addEventListener('change', event => {
    ...
    <div :class="{
        ...
        <div class="p-2 min-h-screen select-none">
            ...
            {{-- Body --}}
            <div class="relative grid grid-cols-1 size-full lg:grid-cols-6 gap-4">

                {{-- List of Archives --}}
                <div class="relative lg:col-span-3 size-full rounded h-[90vh] bg-white shadow">
                        ...
                        <div class="mx-2 flex items-center justify-end">
                            ...
                            <div class="relative me-2">
                                <div class="absolute inset-y-0 start-0 flex items-center ps-2 pointer-events-none">
                                    ...
                                    {{-- Loading Icon --}}
                                    <svg class="size-3 text-indigo-700 animate-spin" wire:loading
                                        wire:target="searchArchives" xmlns="http://www.w3.org/2000/svg" fill="none"
                                        viewBox="0 0 24 24">
                                    ...
                                    {{-- Search Icon --}}
                                    <svg class="size-3 text-indigo-500" wire:loading.remove wire:target="searchArchives"
                            ...
                            <table class="relative w-full text-sm text-left text-indigo-1100 whitespace-nowrap">
                                ...
                                <tbody class="relative text-xs">
                                    @foreach ($this->archives as $key => $archive)
                                        <tr wire:key="archive-{{ $key }}"
                                            wire:loading.class="pointer-events-none" wire:target=""
                                            wire:click="selectRow({{ $key }}, '{{ encrypt($archive['id']) }}')"
                                            ...
                                            <td
                                                class="absolute h-full w-1 left-0
                                            ...
                                            <td class="flex items-center p-1">
                                                {{-- Restore Button --}}
                                                <button type="button"
                                                    @click.stop="$wire.selectRestore('{{ encrypt($archive['id']) }}');"
                                                    id="restoreButton-{{ $key }}"
                                                ...
                                                <button type="button"
                                                    @click.stop="$wire.selectDelete('{{ encrypt($archive['id']) }}');"
                                                    id="deleteButton-{{ $key }}"
                                                    class="flex items-center justify-center z-0 mx-1 p-1 font-medium rounded outline-none duration-200 ease-in-out 
                                                    {{ $selectedRowKey === $key
                                                        ? 'hover:bg-indigo-700 focus:bg-indigo-700 text-indigo-900 hover:text-indigo-50 focus:text-indigo-50'
                ...
                {{-- Beneficiary Preview --}}
                <div class="relative lg:col-span-3 flex flex-col h-[90vh] w-full rounded bg-white shadow">
